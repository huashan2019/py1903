...
static const SCH_U8 Amp8546_IICAddr_tab[] = 
{
	0xdc,0xda,0xde,0xd4,
};
...
------------------------------------------------------------------------*/
void Amp_DetectAddress(void)
{
	SCH_U8 i;
	for(i=0;i<4;i++)
	{
		if(I2C0_IsFindAddr(Amp8546_IICAddr_tab[i]))
		{
			AmpAddr = Amp8546_IICAddr_tab[i];
...
{ 
	...
	I2C0_Tx(AmpAddr, tbl_Amp_Init, sizeof(tbl_Amp_Init));
}
